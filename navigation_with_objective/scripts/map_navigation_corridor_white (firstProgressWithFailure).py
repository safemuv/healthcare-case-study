...
class Map_navigation():
	...
	def __init__(self):
		...
		self.modeFailureInjection=1 #this flag activates and deactivates the mode failure injection (0--> off; 1-->on)
		...
		#self.timePerTaskCalulator(0.5)
		#self.retryTask2()
		#self.getRoom()this method wont be used any longer
		self.run()
	...
	def getRoom(self):
		#indexRoom = self.gateway.jvm.java.util.ArrayList()#Defining type	
		#indexRoom=self.intermediary.getCoordinates(self.robotName)#index of room to serve or numOfRoom
		...
		rospy.sleep(1.)#wait for the message from socket
		indexRoom=[3,1,4]
		#print(indexRoom)
		#print(self.mySetOfRooms)
		rc=self.roomsToServe(indexRoom)

	
		setDistance=[]#distances from home to each of the assigned rooms
		#will assigne home point depending on the robot's name
		...
		#having the coordinates of the room in callbackPose() will be calculated d(currentLocation,furthestRoom)
		#self.sendGoals(rc)#sends the set of goals to be executed
	...
	def client_program(self):
	    ...
	    while message.lower().strip() != 'bye':
	        ...
	        d=d.split(',')#then split, otherwhise ',' are considered characters
	        d=deque(d)
	        self.mySetOfRooms=d
		...
		while True:
			...
			while self.mySetOfRooms:
			 	if(self.isFailureInjected):
					#sprint("the failure has been injected ")
					self.failure=copy.deepcopy(self.mySetOfRooms) #current set of rooms is saved to be later resumed
					#print("back up array")
					#print(self.failure)	
					#print("set of rooms")
					#print(self.mySetOfRooms)
	...
	def readRoomsFile(self):
		with open('/home/jharbin/configurationFiles/test(originales).txt', 'r') as filehandle:
	...
	def sendGoals(self,rc):
		for i in range(0,len(rc)):
			#self.statusCurrentRoom=False#the current status is false and will change only after executing the tasks
			if(self.isFailureInjected):
				last=i
				print("the failure has been injected ")
				for j in range(last,len(rc)):
					self.failure.append(str(rc[j][0])+","+str(rc[j][1])+","+str(rc[j][2]))
				break
				...
				if flag==True:
					...
					if(rc[i][2]==1):
						self.executeTasksRoom1()
						self.currentRoom=str(i)+","+str(rc[i][0])+","+str(rc[i][1])+","+str(rc[i][2])#the coordinates and index of the current room are stored
						self.statusCurrentRoom=True #room whas finished 
					elif(rc[i][2]==2):
						self.executeTasksRoom2()
						self.currentRoom=str(i)+","+str(rc[i][0])+","+str(rc[i][1])+","+str(rc[i][2])#the coordinates and index of the current room are stored
					...
					self.success.append(str(rc[i][0])+","+str(rc[i][1])+","+str(rc[i][2]))
				elif flag==False:
					self.currentRoom=str(i)+","+str(rc[i][0])+","+str(rc[i][1])+","+str(rc[i][2])#the coordinates and index of the current room are stored
					self.statusCurrentRoom=False #means that the robot failed the current room (only when is not reached doesn't takes into account when smth goes wrong with tasks) 
					self.failure.append(str(rc[i][0])+" failure has been injected ")
				#for j in rang, +str(rc[i][1])+","+str(rc[i][2]))
		print(self.success)
		print(self.failure)
			
	def executeTasksRoom1(self):
		print("Type of room 1")
	...
	def triggerClockMonitor(self):
		...
		#this.file="/home/jharbin/configurationFiles/failureIntervals.txt";
		lista=[]
		fileContent= []
		...
		for i in range(0,len(failureI)):
			...
			print("failure must be stopped here!!! MISA")
			print("exito!")
	...
	def stopFailure(self):
		self.isFailureInjected=False
		#self.mySetOfRooms=copy.deepcopy(self.failure)#set of rooms has now the pending goals 
		#tempFailure=self.failure #failure is copied into a temporal array
		#self.failure.clear() #failure is emptied
		#print(self.mySetOfRooms)
		print("Goals are resumed here")
		#self.run()

	def copyDeque(self,dequeO,dequeD):
		for e in dequeO:
			dequeD.append(e)
		print("deqeu en copy function")
		print(dequeD)
		return dequeD
