...
class RobotHomeState(Enum):
    NOT_STARTED = 1
    ON_MISSION = 2
    GOING_HOME_NO_WORK = 3
    GOING_HOME_NO_ENERGY = 4
...
class Map_navigation():
    ...
    def __init__(self):
        ...
        self.home = RobotHomeState.NOT_STARTED        # is the robot at home (or set to go home)
        ...
        rospy.Subscriber("go_home", std_msgs.msg.String, self.return_home_callback)
    ...
    def work_assignment_callback(self,msg):
        ...
        # Should accept work when in state GOING_HOME_NO_WORK - not when the minimum energy
        # threshold is met
        if (not self.batteryEmpty and (not self.home == RobotHomeState.GOING_HOME_NO_ENERGY)):
    ...
    def return_home_callback(self,msg):
        print("return_home_callback on " + self.robotName + " : " + msg.data)
        if ((not self.home == RobotHomeState.GOING_HOME_NO_ENERGY) and not self.batteryEmpty):
            self.mySetOfRooms=[]
            self.ac.cancel_goal()
            self.ac.cancel_all_goals()
            flag=self.moveToGoal(self.homeCor[0], self.homeCor[1])
            self.home = RobotHomeState.GOING_HOME_NO_ENERGY
    ...
    def roomsEmulator(self):
        print("roomsEmulator replaced with CI work assignments")
...
#			flattenParams =  ",".join(map(str, self.parameters))#map(','.join, self.parameters)
    ...
    def run(self):
        self.home = RobotHomeState.NOT_STARTED
        while True:
            ...
            while self.mySetOfRooms:
                if(not self.isFailureInjected):#if failure is not injected then do visit rooms	
                    self.home = RobotHomeState.ON_MISSION
                    ...
                    if flag==True:
                        ...
                        if (roomExecuted == True):
                            ...
                            ### JRH: publish TO /roomsCompleted here - only if doing the mission currently
                            if ((self.home == RobotHomeState.ON_MISSION) and (not self.batteryEmpty)):
            ...
            if ((self.home == RobotHomeState.ON_MISSION) and not self.batteryEmpty):
                flag=self.moveToGoal(self.homeCor[0], self.homeCor[1])
                self.home = RobotHomeState.GOING_HOME_NO_WORK
